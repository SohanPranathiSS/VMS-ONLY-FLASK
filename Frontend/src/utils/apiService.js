...
const API_BASE_URL = process.env.REACT_APP_API_BASE_URL || 'https://visitors.pranathiss.com/api';
 ...
 */
// Build a full URL ensuring the '/api' prefix exists when base URL doesn't include it
const buildUrl = (endpoint) => {
  const base = (API_BASE_URL || '').replace(/\/+$/, '');
  const ep = endpoint.startsWith('/') ? endpoint : `/${endpoint}`;
  const baseHasApi = /(^|\/)api$/i.test(base);
  const epHasApi = ep.toLowerCase().startsWith('/api/');
  const needsApi = !baseHasApi && !epHasApi;
  return `${base}${needsApi ? '/api' : ''}${ep}`;
};
...
const request = async (endpoint, options = {}) => {
  ...
  const response = await fetch(url, { ...options, headers });
    ...
    if (!response.ok) {
        // For duplicate check-in errors, include additional details
        if (response.status === 409 && responseData.error === 'DUPLICATE_CHECKIN') {
            const error = new Error(responseData.message);
            error.isDuplicateCheckIn = true;
            error.existingVisitId = responseData.existing_visit_id;
            error.existingCheckInTime = responseData.existing_checkin_time;
            throw error;
        }
...
// Forgot password: check if email is verified
export const checkEmailVerified = (email) => {
  return request('/forgot-password/check', {
    method: 'POST',
    body: JSON.stringify({ email }),
  });
};

// Forgot password: reset using verified email
export const resetPasswordByEmail = (email, new_password, confirm_password) => {
...
// Get current user's company info (includes subscription_start_date and subscription_end_date)
...
/** Create a support ticket for current user/company */
...
/** List non-closed tickets for current user/company */
